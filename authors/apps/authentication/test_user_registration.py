...
class RegistrationTestCase(APITestCase):
    ...
    def test_empty_username(self):
        ...
        response = self.client.post(self.signup_url,
        self.signup_empty_username,format="json")
    ...
    def test_empty_email(self):
        ...
        response = self.client.post(self.signup_url,
        self.signup_empty_email,format="json")
        self.assertEqual(response.status_code,status.HTTP_400_BAD_REQUEST)
    def test_empty_password(self):
        '''Test register user with an empty password'''
        response = self.client.post(self.signup_url,
        self.signup_empty_password, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    
    def test_register_duplicate_email(self):
        '''Test register user with a duplicate email'''
        self.client.post(self.signup_url,self.duplicate_email,format="json")
        response =self.client.post(self.signup_url,self.duplicate_email2,format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
